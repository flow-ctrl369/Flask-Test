...
def send_email_notification(todo):
    ...
    try:
        ...
        Category: {todo.category or 'No category'}
...
def index():
    ...
    if category_filter:
        todos = Todo.query.filter_by(category=category_filter).order_by(Todo.created_at.desc()).all()
    else:
        todos = Todo.query.order_by(Todo.created_at.desc()).all()
    
    # Get all unique categories for the filter dropdown
    categories = db.session.query(Todo.category).distinct().filter(Todo.category.isnot(None)).all()
    categories = [cat[0] for cat in categories]
    
    today = datetime.now().date()
    return render_template('index.html', todos=todos, today=today, categories=categories, selected_category=category_filter)
...
def add():
    ...
    if request.method == 'POST':
        ...
        todo = Todo(title=title, description=description, category=category, due_date=due_date)
        db.session.add(todo)
        db.session.commit()
